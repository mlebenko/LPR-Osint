...
// Утилита: лёгкий Markdown → HTML (только для уже проверенного текста модели)
...
// ===============================
// Главный компонент — поэкранный мастер: Шаг 1 → Шаг 2 → Шаг 3
// ===============================
export default function Page() {
  // Экраны (по макетам):
  // s1-input  → экран ввода запроса
  // s1-choose → экран выбора компании (если найдено >1 кандидата)
  // s2-people → экран ЛПР
  // s3-profiles → экран Портретов
  ...
  const [selectedPeopleIdx, setSelectedPeopleIdx] = useState<number[]>([]); // отмеченные на шаге 2

  // Портреты
  const [productInfo, setProductInfo] = useState("");
  const [profilesMd, setProfilesMd] = useState<string>("");
  const [activeProfileIdx, setActiveProfileIdx] = useState<number | null>(null); // индекс в массиве выбранных

  // Тех. логи (можно скрыть в проде)
  const [debugText, setDebugText] = useState<string>("");

  // Разбиение Markdown на карточки по заголовкам (### Имя — Должность)
  ...
  // Возвращает HTML-портрет по ФИО из массива профилей
  ...
  async function identifyCompany() {
    ...
    try {
      ...
      // Развилка: INN → сразу шаг 2; Название → выбрать компанию
      if (mode === "inn") {
        // Если пришёл один кандидат — используем его; иначе пробуем ИНН из ввода
        let inn = data.candidates?.[0]?.inn?.trim() || query.trim();
        let name = data.candidates?.[0]?.name || "";
        let region = data.candidates?.[0]?.region || "";
        let sourceDomain = data.candidates?.[0]?.source ? domainFromUrl(data.candidates[0].source) : "";
        await gotoPeople({ inn, name, region, sourceDomain });
      } else {
        if (data.candidates?.length === 1) {
          setChosenIdx(0);
          const c = data.candidates[0];
          await gotoPeople({ inn: c.inn, name: c.name, region: c.region || "", sourceDomain: domainFromUrl(c.source) });
        } else {
          setScreen("s1-choose");
        }
      }
  ...
  async function gotoPeople({ inn, name, region, sourceDomain }: { inn: string; name?: string; region?: string; sourceDomain?: string; }) {
    setScreen("s2-people");
    ...
    try {
      const res = await fetch("/api/find-lpr", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ inn, companyName: name || "", region: region || "", sourceDomain: sourceDomain || "" }),
      });
      if (!res.ok) throw new Error(await res.text());
      const data = await res.json();
  ...
  async function makeProfiles() {
    ...
    try {
      ...
      // Переходим на шаг 3 и активируем первую карточку из выбранных
  ...
  function resetAll() {
    ...
    setCandidates([]); setChosenIdx(null);
    setPeople([]); setSelectedPeopleIdx([]);
    setProductInfo(""); setProfilesMd("");
    setActiveProfileIdx(null);
    setError(null); setDebugText("");
  }

  // ===============================
  // UI — по макетам, с иконками Google Fonts (Material Symbols)
  // ===============================
  return (
    <main style={{ display: "grid", gap: 16 }}>
        ...
        <Screen>
          <Card>
            <div style={{ display: "grid", gap: 12 }}>
              ...
              <Seg> 
                <SegTab active={mode === "inn"} onClick={() => setMode("inn")}>
                  <Icon name="badge" /> По ИНН
                </SegTab>
                <SegTab active={mode === "name"} onClick={() => setMode("name")}>
                  <Icon name="business" /> По названию
                </SegTab>
              </Seg>
            </div>
        ...
        <Screen>
          <Card>
                ...
                <label key={i} style={companyItem(chosenIdx === i)}>
                  ...
                  <div style={{ display: "grid", gap: 6 }}>
                    ...
                    <div style={{ display: "flex", gap: 12, flexWrap: "wrap", fontSize: 13 }}>
                      ...
                      <span>
                        <Icon name="public" /> Источник: <a href={c.source} target="_blank" rel="noreferrer">{domainFromUrl(c.source)}</a>
                      </span>
                    </div>
                  </div>
                </label>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <Primary
                disabled={chosenIdx === null || loading}
                onClick={() => {
                  if (chosenIdx === null) return;
                  const c = candidates[chosenIdx];
                  gotoPeople({ inn: c.inn, name: c.name, region: c.region, sourceDomain: domainFromUrl(c.source) });
                }}
              >
                Продолжить <Icon name="chevron_right" />
              </Primary>
      ...
      {/* Экран 2 — ЛПР/ЛВПР для выбранной компании */}
      {screen === "s2-people" && (
        <Screen>
          <Card>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              ...
              <div>
                {chosen ? (
                  <Badge>
                    <Icon name="domain" /> {chosen.name} · ИНН {chosen.inn}
                  </Badge>
                ) : null}
              </div>
            </div>
            ...
            {people.length === 0 ? (
              <p style={{ opacity: 0.8, marginTop: 8 }}>{loading ? "Ищу персоналии…" : "Нет данных. Попробуйте другой запрос."}</p>
            ) : (
                  ...
                  <div key={i} style={personCard(selectedPeopleIdx.includes(i))}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {p.sources?.slice(0, 3).map((s, j) => (
                        <a key={j} href={s.url} target="_blank" rel="noreferrer" style={chip}>
                          {s.label || domainFromUrl(s.url)}
                        </a>
                      ))}
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 6 }}>
                      <button
                        ...
                        title="Открыть детальную карточку на шаге 3"
                      >
                        <Icon name="visibility" /> Детальная информация
                      </button>
      ...
      {/* Экран 3 — Портреты: показываем ТОЛЬКО выбранных */}
      {screen === "s3-profiles" && (
        <Screen>
          <Card>
            <Title>Портрет ЛПР</Title>

            {/* Список выбранных персон слева */}
              ...
              <div>
                ...
                <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "grid", gap: 8 }}>
                  {selectedPeopleIdx.map((idx, k) => (
                    <li key={k}>
                      <button
                        onClick={() => setActiveProfileIdx(k)}
                        style={{ ...listBtn, ...(k === activeProfileIdx ? listBtnActive : null) }}
                      >
                        <span style={{ fontWeight: 700 }}>{people[idx]?.full_name || "—"}</span>
                        <span style={{ opacity: 0.8 }}> — {people[idx]?.role_title || ""}</span>
                      </button>
                    </li>
                  ))}
                </ul>
...
// ===============================
// Презентационные компоненты (без Tailwind, чтобы не ломать билд)
// ===============================
function Header() {
  return (
...
function Stepper({ screen }: { screen: "s1-input" | "s1-choose" | "s2-people" | "s3-profiles" }) {
  ...
  const item = (n: number, label: string, active: boolean) => (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div
        style={{
          width: 28, height: 28, borderRadius: "50%", lineHeight: "28px", textAlign: "center",
          fontWeight: 700, color: active ? "#fff" : "#0f172a", background: active ? "#2563eb" : "#e2e8f0",
        }}
      >{n}</div>
  ...
  return (
    <nav style={{ display: "flex", gap: 16, alignItems: "center" }}>
      {item(1, cur.n === 1 ? cur.label : "Поиск компании", cur.n >= 1)}
      <span style={{ opacity: 0.5 }}>—</span>
      {item(2, cur.n === 2 ? cur.label : "Поиск ЛПР/ЛВПР", cur.n >= 2)}
      <span style={{ opacity: 0.5 }}>—</span>
...
function Screen({ children }: { children: React.ReactNode }) {
  return (
    <section style={{ minHeight: "70vh", display: "grid", alignContent: "start" }}>{children}</section>
  );
}

function Card({ children }: { children: React.ReactNode }) {
  return (
    <section style={{ background: "#fff", padding: 16, borderRadius: 14, boxShadow: "0 1px 2px rgba(0,0,0,0.06)" }}>{children}</section>
  );
}

function Title({ children }: { children: React.ReactNode }) {
  return <h2 style={{ fontSize: 18, fontWeight: 800, margin: 0, marginBottom: 8 }}>{children}</h2>;
}

function BlockTitle({ children }: { children: React.ReactNode }) {
  return <div style={{ marginTop: 8, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}>{children}</div>;
}

function Label({ children }: { children: React.ReactNode }) {
  return <label style={{ fontSize: 14, opacity: 0.8 }}>{children}</label>;
}

function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} style={{ flex: 1, padding: "12px 14px", border: "1px solid #e2e8f0", borderRadius: 12, fontSize: 14 }} />;
}

function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...props} style={{ width: "100%", minHeight: 120, padding: 12, border: "1px solid #e2e8f0", borderRadius: 12 }} />;
}

function Primary({ children, ...rest }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button {...rest} style={{ background: "#2563eb", color: "#fff", border: "none", padding: "12px 16px", borderRadius: 12, cursor: "pointer", fontWeight: 700 }}>
      {children}
    </button>
  );
}

function Ghost({ children, ...rest }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button {...rest} style={{ background: "#f1f5f9", color: "#0f172a", border: "none", padding: "10px 14px", borderRadius: 12, cursor: "pointer", fontWeight: 600 }}>
      {children}
    </button>
  );
}

function Outline({ children, ...rest }: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    <button {...rest} style={{ background: "transparent", color: "#2563eb", border: "1px solid #bfdbfe", padding: "8px 12px", borderRadius: 10, cursor: "pointer", fontWeight: 700 }}>
      {children}
    </button>
  );
}
...
function Seg({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ display: "inline-flex", background: "#e2e8f0", borderRadius: 12, padding: 4, width: "fit-content" }}>{children}</div>
  );
}

function SegTab({ active, onClick, children }: { active?: boolean; onClick?: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} style={{ border: "none", padding: "8px 12px", borderRadius: 8, background: active ? "#ffffff" : "transparent", boxShadow: active ? "0 1px 2px rgba(0,0,0,0.08)" : "none", cursor: "pointer", fontWeight: active ? 700 : 500 }}>
      {children}
    </button>
  );
}
...
function Badge({ children }: { children: React.ReactNode }) {
  return <span style={{ background: "#e0f2fe", color: "#075985", borderRadius: 999, padding: "2px 8px", fontSize: 12, fontWeight: 700, display: "inline-flex", alignItems: "center", gap: 6 }}>{children}</span>;
}

function AvatarLetter({ name }: { name: string }) {
  const letter = (name || "?").trim().charAt(0).toUpperCase();
  return (
    <div style={{ width: 40, height: 40, borderRadius: 12, background: "#eff6ff", color: "#1d4ed8", display: "grid", placeItems: "center", fontWeight: 800 }}>{letter}</div>
  );
}
...
function DetailPanel({ person, html }: { person: Person; html: string | null }) {
  return (
    <div style={{ display: "grid", gap: 10 }}>
      ...
      <BlockTitle><Icon name="link" /> Источники</BlockTitle>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        {person.sources?.slice(0, 5).map((s, i) => (
          <a key={i} href={s.url} target="_blank" rel="noreferrer" style={chip}>{s.label || domainFromUrl(s.url)}</a>
        ))}
      </div>
      ...
      {html ? (
        ...
        <div style={{ opacity: 0.8 }}>
          Карточка появится после генерации портретов. Разделы: Зоны ответственности, Боли/задачи и KPI/OKR, Триггеры, Сообщение.
        </div>
      )}
    </div>
  );
...
function Icon({ name, size = 20 }: { name: string; size?: number }) {
  return (
    <span className="material-symbols-outlined" style={{ fontSize: size, lineHeight: 1, verticalAlign: "-4px" }}>{name}</span>
  );
}
